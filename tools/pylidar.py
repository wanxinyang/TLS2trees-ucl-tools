...
def get_args():
    ...
    argparser.add_argument('--min_zenith', metavar='FLOAT', type=float, default=[0.], nargs='+',
        help='Minimum zenith angle to use for input file')
    argparser.add_argument('--max_zenith', metavar='FLOAT', type=float, default=[70.], nargs='+',
    ...
    argparser.add_argument('-q','--query_str', metavar='STR', type=str, nargs='*', default=None,
...
def run():
    ...
    x,y,z,r = plant_profile.get_min_z_grid(riegl_files, 
                                           args.transform_file,
                                           args.grid_extent, 
    ...
    vpp = plant_profile.Jupp2009(hres=args.height_resolution, 
                                 zres=args.zenith_resolution, 
                                 ares=args.azimuth_resolution, 
                                 min_z=min(args.min_zenith), 
                                 max_z=max(args.max_zenith), 
                                 min_h=0, 
    ...
    for i in range(n_pos):
        ...
        vpp.add_riegl_scan_position(args.input_file[i], 
                                    args.transform_file[i], 
                                    sensor_height=None,
                                    rdbx_file=None, 
                                    method='WEIGHTED', 
                                    min_zenith=35, 
    ...
    #export profile
    vpp_profile=vpp.exportPlantProfiles()
    vpp_profile.to_csv(args.outfile)
   
    pgap_profile=vpp.exportPgapProfiles()
    pgap_profile.to_csv(args.outfile.replace('pylidar', 'pgap'))
